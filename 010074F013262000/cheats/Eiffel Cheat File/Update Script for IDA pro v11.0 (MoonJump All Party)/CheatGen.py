...
################################ START ######################################
    ...
    cheatName = 'Movement Speed (2x) hold Y' if english else '移動速度 (2x) hold Y' 
    show('%d%s %s'%(cheatCnt,chr(idx),cheatName)); delayOutput += '\n[#%02d%s %s]\n'%(cheatCnt,chr(idx),cheatName)
    Hack(cheatAddr,('FMOV S2, #2.0','FMUL S1, S1, S2'),useButton='Y')

    idx+=1
    cheatName = 'Movement Speed (3x) hold Y' if english else '移動速度 (3x) hold Y' 
    show('%d%s %s'%(cheatCnt,chr(idx),cheatName)); delayOutput += '\n[#%02d%s %s]\n'%(cheatCnt,chr(idx),cheatName)
    Hack(cheatAddr,('FMOV S2, #3.0','FMUL S1, S1, S2'), False, useButton='Y')

    idx+=1
    cheatName = 'Movement Speed (5x) hold Y' if english else '移動速度 (5x) hold Y' 
    show('%d%s %s'%(cheatCnt,chr(idx),cheatName)); delayOutput += '\n[#%02d%s %s]\n'%(cheatCnt,chr(idx),cheatName)
    Hack(cheatAddr,('FMOV S2, #5.0','FMUL S1, S1, S2'), False, useButton='Y')

    idx+=1
    cheatName = 'Movement Speed (8x) hold Y ★' if english else '移動速度 (8x) hold Y ★' 
    show('%d%s %s'%(cheatCnt,chr(idx),cheatName)); delayOutput += '\n[#%02d%s %s]\n'%(cheatCnt,chr(idx),cheatName)
    Hack(cheatAddr,('FMOV S2, #8.0','FMUL S1, S1, S2'), False, useButton='Y')

    idx+=1
    cheatName = 'Movement Speed (13x) hold Y' if english else '移動速度 (13x) hold Y' 
    show('%d%s %s'%(cheatCnt,chr(idx),cheatName)); delayOutput += '\n[#%02d%s %s]\n'%(cheatCnt,chr(idx),cheatName)
    ...
    cheatCnt+=1 ; idx=ord('a')
    cheatName = 'FriendShip Points Multiplier (3X)' if english else '羈伴點數增加倍率 (3X)' 
    show('%d%s %s'%(cheatCnt,chr(idx),cheatName)); delayOutput += '\n[#%02d%s %s]\n'%(cheatCnt,chr(idx),cheatName)
    Hack(cheatAddr, 'ADD W20, W2, W2, LSL#1')
    
    idx+=1
    cheatName = 'FriendShip Points Multiplier (5X) ★' if english else '羈伴點數增加倍率 (5X) ★' 
    show('%d%s %s'%(cheatCnt,chr(idx),cheatName)); delayOutput += '\n[#%02d%s %s]\n'%(cheatCnt,chr(idx),cheatName)
    Hack(cheatAddr, 'ADD W20, W2, W2, LSL#2',False)
    
    idx+=1
    cheatName = 'FriendShip Points Multiplier (8X)' if english else '羈伴點數增加倍率 (8X)' 
    show('%d%s %s'%(cheatCnt,chr(idx),cheatName)); delayOutput += '\n[#%02d%s %s]\n'%(cheatCnt,chr(idx),cheatName)
    ...
    cheatCnt+=1
    cheatName = 'Items are not required for Quest ★' if english else '任務中,無視道具物品要求 ★' 
    show('%d. %s'%(cheatCnt,cheatName)); delayOutput += '\n[#%02d. %s]\n'%(cheatCnt,cheatName)
